...
void PSO::CreateLinePSO(
	...
	rootParameters[0].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;    // CBVとして設定
	rootParameters[0].Descriptor.ShaderRegister = 0;                    // レジスタ番号b0
	// rootParameters[0].DescriptorRangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV; // ★この行を削除★
	rootParameters[0].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL; // ピクセルシェーダーから参照

	// WVP用 (b1)
	rootParameters[1].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;    // CBVとして設定
	rootParameters[1].Descriptor.ShaderRegister = 1;                    // レジスタ番号b1
	rootParameters[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX; // 頂点シェーダーから参照
	...
	hr = device->CreateRootSignature(0, rootSignatureBlob->GetBufferPointer(), rootSignatureBlob->GetBufferSize(), IID_PPV_ARGS(&lineRootSignature_));
	assert(SUCCEEDED(hr));

	// InputLayout (Line.VS.hlsl に合わせる)
	...
	// Shaderをコンパイルする
	Microsoft::WRL::ComPtr<IDxcBlob> lineVsBlob = CompileShader(L"Line.VS.hlsl", L"vs_6_0", dxcUtils.Get(), dxcCompiler.Get(), includeHandler.Get()/*, logStream*/);
	assert(lineVsBlob != nullptr);

	Microsoft::WRL::ComPtr<IDxcBlob> linePsBlob = CompileShader(L"Line.PS.hlsl", L"ps_6_0", dxcUtils.Get(), dxcCompiler.Get(), includeHandler.Get()/*, logStream*/);
	assert(linePsBlob != nullptr);

	// BlendState の設定 (アルファブレンドを考慮する場合)
	D3D12_BLEND_DESC blendDesc{};
	...
	D3D12_RASTERIZER_DESC rasterizerDesc{};
	rasterizerDesc.FillMode = D3D12_FILL_MODE_SOLID;
	rasterizerDesc.CullMode = D3D12_CULL_MODE_NONE; // 線なのでカリングしない

	// DepthStencilState の設定 (線なのでデプスもステンシルも無効)
	D3D12_DEPTH_STENCIL_DESC depthStencilDesc{};
	depthStencilDesc.DepthEnable = FALSE; // デプス無効
	...
	graphicsPipelineStateDesc.VS = { lineVsBlob->GetBufferPointer(), lineVsBlob->GetBufferSize() };
	graphicsPipelineStateDesc.PS = { linePsBlob->GetBufferPointer(), linePsBlob->GetBufferSize() };
	graphicsPipelineStateDesc.BlendState = blendDesc;
	graphicsPipelineStateDesc.RasterizerState = rasterizerDesc;
	graphicsPipelineStateDesc.DepthStencilState = depthStencilDesc; // DepthStencilStateを設定
	graphicsPipelineStateDesc.InputLayout = inputLayoutDesc;
	graphicsPipelineStateDesc.PrimitiveTopologyType = D3D12_PRIMITIVE_TOPOLOGY_TYPE_LINE; // 線を描画するために変更
	graphicsPipelineStateDesc.NumRenderTargets = 1;
	...
	// PSO の作成
	hr = device->CreateGraphicsPipelineState(&graphicsPipelineStateDesc, IID_PPV_ARGS(&linePipelineState_));
	assert(SUCCEEDED(hr));
}